...
PDEPricer::PDEPricer(double maturity, int time_steps, double multiplier, double volatility, int space_steps,
    ...
    boundary_conditions_upper = std::vector<double>(time_steps + 1, multiplier * volatility * exp(risk_free_rate * maturity));
...
void PDEPricer::initialize_matrices(double dt, double dx, double volatility, double risk_free_rate) {
    ...
    for (size_t i = 1; i < n - 1; ++i) {
        ...
        a_matrix[i][i] = 1.0 + sigma2 * x * x * dt / (dx * dx) + risk_free_rate * dt;
        b_matrix[i][i - 1] = -0.5 * sigma2 * x * x * dt / (dx * dx);
        b_matrix[i][i + 1] = -0.5 * sigma2 * x * x * dt / (dx * dx);
        c_matrix[i][i] = -risk_free_rate * dt;
        d_matrix[i][i] = 0.0; // d_matrix is unused in this simple scheme
...
std::vector<double> PDEPricer::solve() const {
    ...
    for (int t = time_steps - 1; t >= 0; --t) {
        ...
        for (int i = 1; i < space_steps - 1; ++i) {
            next_solution[i] = (b_matrix[i][i - 1] * current_solution[i - 1] +
                                a_matrix[i][i] * current_solution[i] +
